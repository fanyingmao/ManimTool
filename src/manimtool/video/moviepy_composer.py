...
实现要点：
    - 每个 scene 用 ImageClip × AudioFileClip 对齐到 TTS 时长，确保"读到对应内容才出现对应内容"
    - 顶部叠加章节进度条 + 章节标签条（基于全片累计进度，反映当前小节进度）
...
except ImportError:  # pragma: no cover - 兼容 moviepy 2.x
...
def _style_from_config(config: object, width: int, height: int) -> OverlayStyle:
    return OverlayStyle(
        width=width,
        height=height,
...
class MoviePyComposer(BaseComposer):
        ...
        try:
            ...
            if transition == "fade" and transition_d > 0 and len(scene_clips) > 1:
                ...
                for c in scene_clips[1:]:
                    faded.append(c.crossfadein(transition_d))
                final = concatenate_videoclips(faded, method="compose", padding=-transition_d)
            else:
    ...
    def _build_scene_clip(
        ...
        *,
        ...
        bg_image = composite_image_on_canvas(
            ...
            padding_top=getattr(self.config, "image_padding_top", 140),
            padding_bottom=getattr(self.config, "image_padding_bottom", 200),
        )
        bg_array = np.array(bg_image)
        base = ImageClip(bg_array, duration=duration)

        layers: list[object] = [base]
    ...
    def _build_progress_clip(
        ...
        *,
        ...
        这样可以保留 RGBA 透明通道（MoviePy 1.x 的 ``ImageSequenceClip``
        会丢弃 alpha 通道，所以必须分段叠加）。
        """
        ...
        for i in range(n_segments):
            ...
            try:
                seg = seg.set_position(("center", 0)).set_start(i * seg_dur)
            except AttributeError:  # moviepy 2.x
                seg = seg.with_position(("center", 0)).with_start(i * seg_dur)
            clips.append(seg)
        bar_w, bar_h = bar.size
        composite = CompositeVideoClip(clips, size=(style.width, bar_h)).set_duration(duration)
        try:
            composite = composite.set_position(("center", 0))
        except AttributeError:
            composite = composite.with_position(("center", 0))
        return composite
    ...
    def _build_subtitle_clips(
        ...
        for cue in art.tts.cues:
            ...
            sub_clip = ImageClip(arr, duration=d)
            try:
                y = style.height - sub_img.size[1] - bottom_margin
                sub_clip = sub_clip.set_position(("center", y)).set_start(cue.start)
            except AttributeError:  # moviepy 2.x
                sub_clip = sub_clip.with_position(("center", y)).with_start(cue.start)
